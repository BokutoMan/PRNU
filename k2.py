...
def evaluate_prnu(prnu_database, test_images_by_camera):
    """
    评估PRNU识别系统的性能。

    参数:
    prnu_database (dict): PRNU数据库，其中键是相机型号，值是相应的PRNU。
    test_images_by_camera (dict): 测试图像的字典，其中键是相机型号，值是相应的测试图像列表。
    ...
    # 遍历每个相机的测试图像
    for camera, test_images in test_images_by_camera.items():
        camera_correct = 0
...
if __name__ == "__main__":
    ...
    # 评估PRNU识别系统的准确率
    accuracy = evaluate_prnu(prnu_database, test_images_by_camera)
