...
def build_prnu_database(base_path, test_ratio=0.2, target_size=(512, 512)):
    ...
    for camera_folder in os.listdir(base_path):
        ...
        if os.path.isdir(camera_path):
            ...
            test_images_with_camera[camera_folder] = test_images
    
    # 返回构建好的PRNU数据库和测试图像字典
    return prnu_database, test_images_with_camera


# 主程序
def main(base_path, test_ratio, target_size):
    # 构建PRNU数据库和测试图像字典
    prnu_database, test_images_with_camera = build_prnu_database(base_path, test_ratio, target_size)
    ...
    # 将测试图像字典保存到文件
    with open('test_images_with_camera.pkl', 'wb') as f:
        pickle.dump(test_images_with_camera, f)
